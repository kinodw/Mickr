  ...
  class Animator extends EventEmitter{
    ...
    initAnimation(op){
      ...
      this.animations = {
        ...
        centering: (position, ratio=3.0, duration) => {
          var tl = new TimelineLite();
  ...
  class MickrSky extends Sky{
    ...
    addCloud(option){
      ...
      if(option.around){cloud.animator.addGoAround(option);}
        ...
        if(option.random) cloud.setPosition(generateRandomInt(0, w-150), generateRandomInt(0, h-150));
        if(!option.immortal){
          console.log("vanish", option.immortal);
